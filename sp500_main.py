...
sentiment = pd.DataFrame(columns=['ticker', 'filing_date', 'link', 'item1a', 'item7', 'item7a'])

for cid_idx, row in cid_10k_500.iterrows():
    ticker, company_Name, Sector, cid, company_name, file, filing_date, text_loc, _ = row

    sec_prefix = "https://www.sec.gov/Archives/"

    link = sec_prefix + text_loc
    results = {'ticker': ticker, 'filing_date': filing_date, 'link': link}
    ...
    if not dict_items:
        ...
        for name, txt in dict_items.items():
            ...
            sentiment_df = sentiment_df.merge(w_sentiment, how='left', on='sentence')
